...
final class PoseRenderer {
    ...
    func animation(named name: String, fps: Double, loops: Bool) -> SpriteAnimation {
        let frames = animationFrames(named: name, in: pack)
        if !frames.isEmpty {
            return SpriteAnimation(frames: frames, fps: fps, loops: loops)
        }

        if let fallbackPack {
            let fallbackFrames = animationFrames(named: name, in: fallbackPack)
            if !fallbackFrames.isEmpty {
                return SpriteAnimation(frames: fallbackFrames, fps: fps, loops: loops)
            }
        }

        let fallback = firstImage(for: .resting).map { [$0] } ?? []
        return SpriteAnimation(frames: fallback, fps: fps, loops: loops)
    }

    func hasAnimation(named name: String, includeFallback: Bool = true) -> Bool {
        if !animationFrames(named: name, in: pack).isEmpty {
            return true
        }
        guard includeFallback, let fallbackPack else { return false }
        return !animationFrames(named: name, in: fallbackPack).isEmpty
    }

    func animationSourcePack(named name: String) -> CatAssetPack? {
        if !animationFrames(named: name, in: pack).isEmpty {
            return pack
        }
        if let fallbackPack, !animationFrames(named: name, in: fallbackPack).isEmpty {
            return fallbackPack
        }
        return nil
    }
    ...
    private func walkFramesFromDirectory(in pack: CatAssetPack) -> [NSImage] {
        animationFrames(named: "walk", in: pack)
    }

    private func animationFrames(named name: String, in pack: CatAssetPack) -> [NSImage] {
        guard let urls = try? fileManager.contentsOfDirectory(
            at: pack.animationDirectoryURL(named: name),
            includingPropertiesForKeys: [.isRegularFileKey],
            options: [.skipsHiddenFiles]
        ) else {
            return []
