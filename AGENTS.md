...
3. Initialize the pack with `python3 Tools/petpack.py init`. Supported first-class species are `cat`, `dog`, `rabbit` and `ferret`; use `other` for another animal and derive its safe signature action from the videos.
   Users who prefer a GUI may launch `python3 Tools/pet_studio.py`; preserve its privacy rule that local reference paths and source media never enter an exported pack.
...
10. Populate manifest v2 personality from observed behavior when video evidence exists. Keep all meters gentle and non-punitive, and declare optional toys/extensions with pack-local paths only.
