...
ZIP_PATH="MiaoXinxin.zip"
PACKAGED_APP_NAME="喵心心.app"

echo "Clean building Miao Xinxin for packaging..."
xcodebuild \
  -project "$PROJECT" \
  -scheme "$SCHEME" \
...
PACKAGE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/miaoxinxin-package.XXXXXX")"
trap 'rm -rf "$PACKAGE_DIR"' EXIT

echo "Preparing release package..."
ditto --norsrc "$APP_PATH" "$PACKAGE_DIR/$PACKAGED_APP_NAME"
echo "Applying an ad-hoc signature to the packaged app..."
codesign --force --sign - --timestamp=none "$PACKAGE_DIR/$PACKAGED_APP_NAME"
codesign --verify --strict --verbose=2 "$PACKAGE_DIR/$PACKAGED_APP_NAME"
...
if find "$PACKAGE_DIR/$PACKAGED_APP_NAME" \
  \( -name '.DS_Store' \
  ...
  find "$PACKAGE_DIR/$PACKAGED_APP_NAME" \
    \( -name '.DS_Store' \
       -o -name 'DockCatTests.xctest' \
       -o -name '*.debug.dylib' \
       -o -name '__preview.dylib' \
...
echo "Packing 喵心心.app, CustomizationGuide, README.md, README.en.md, and LICENSE.txt..."
rm -f "$ZIP_PATH"
