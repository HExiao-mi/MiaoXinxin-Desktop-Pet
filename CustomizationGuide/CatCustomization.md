# Cross-platform Pet Customization

The shared pack format now supports cats, dogs, rabbits, ferrets, and other animals on both macOS and Windows. The quickest workflow is to attach photos and videos of one pet in a Codex conversation and ask for a cross-platform desktop pet; `AGENTS.md` defines the completion and QA contract. See `自定义指引.md`, `多物种动作提示词.md`, and `docs/IMPLEMENTATION.md` for the manifest-v2 schema and species-specific motion rules.
